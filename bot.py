...
# ── 6 МЕД-АРХЕТИПОВ ───────────────────────────────────────────────────────────
TYPES = ["Шизоид", "Психастеник", "Эпилептоид", "Истероид", "Неврастеник", "Депрессивный"]

PROFILE = {
    "Шизоид": {
        "alias": "Архетип Мудреца",
        "desc": "Человек смысла и тишины. При стрессе уходит в мысли, тело «отключает».",
        "nervous": "Доминирует неокортекс, слабая связь с телом.",
        "soma": "Шея/плечи, глаза, голова; склонность к бессоннице.",
        "hygiene": "Сенсорное заземление, медленное дыхание, паузы без экрана, тёплый душ.",
        "triggers": "Шум, давление, вторжение в личное пространство, бессмысленные разговоры.",
        "recovery": "Тишина, прогулка, ритуалы (чай, свеча), фокус «чувствую тело».",
        "partner": "Тёплая телесная энергия (гедонистический полюс) — учит расслабляться и радоваться."
    },
    "Психастеник": {
        "alias": "Архетип Искателя",
        "desc": "Тонкая чувствительность и тревожный контроль. Ищет безопасность и ясность.",
        "nervous": "Легко перевозбуждается от неопределённости.",
        "soma": "Диафрагма/ЖКТ, дыхание, сердцебиение.",
        "hygiene": "Дыхание 4–6, план на день, дневник благодарности, тёплая еда.",
        "triggers": "Резкие перемены, критика, хаос в расписании.",
        "recovery": "Стабильные ритуалы, опора на ощущения тела, мягкий поддерживающий диалог.",
        "partner": "Структурный полюс (Воин/Эпилептоид) — даёт опору и форму."
    },
    "Эпилептоид": {
        "alias": "Архетип Воина",
        "desc": "Сила, структура, контроль. Телу нужен безопасный выход энергии.",
        "nervous": "Склонность к мобилизации, высокий тонус.",
        "soma": "Спазмы, челюстное напряжение, гипертония, головные боли.",
        "hygiene": "Физическая разрядка, растяжка, длинный выдох через рот, массаж шеи/плеч.",
        "triggers": "Хаос, отсутствие правил, пассивность окружающих.",
        "recovery": "Порядок, чёткие задачи, спорт, дыхание с акцентом на выдох.",
        "partner": "Тёплый поддерживающий полюс (Заботливый/Депрессивный) — смягчает и наполняет."
    },
    "Истероид": {
        "alias": "Архетип Творца",
        "desc": "Эмоциональная выразительность и потребность быть увиденным. Источник вдохновения.",
        "nervous": "Лимбическая система активна, эмоции быстро вспыхивают.",
        "soma": "Кожа, горло/дыхание; психофизиология голоса и сцены.",
        "hygiene": "Дневник эмоций, «дыхание через сердце», ограничение экранов, практика самоценности.",
    ...
    "Неврастеник": {
        "alias": "Архетип Лидера",
        "desc": "Высокая активность и истощаемость. Быстрый старт — риск выгорания.",
        "nervous": "Колебания тонуса, ресурс заканчивается незаметно.",
        "soma": "Сердце/сосуды, слабость, мышечный тонус.",
        "hygiene": "Режим сна, прогулки без телефона, отказ от многозадачности, утренние растяжки.",
        "triggers": "Переутомление, сверхответственность, постоянный контроль.",
        "recovery": "Планируемые паузы, делегирование, дыхание 4–6.",
        "partner": "Устойчивый «воинский» полюс (Эпилептоид) — помогает дозировать нагрузку."
    },
    "Депрессивный": {
        "alias": "Архетип Заботливого",
        "desc": "Глубокая эмпатия и склонность забывать о себе. Телу нужна мягкая энергия.",
        "nervous": "Снижение тонуса при перегрузе эмоциями.",
        "soma": "Спина/грудь, желудок, склонность к низкому давлению.",
        "hygiene": "Тепло, практики на выдох, «забота о себе» как ритуал, мягкая нагрузка.",
        "triggers": "Вина, одиночество, переизбыток заботы о других.",
        "recovery": "Объятия, признание потребностей, осознанное питание и отдых.",
        "partner": "Собранный лидерский полюс (Неврастеник) — включает движение и ясность."
    },
}
...
async def cmd_start(msg: types.Message):
    ...
    users[uid] = {"step": 0, "scores": defaultdict(int), "finished": False, "result": None, "daily": False}
    await msg.answer(
...
async def handle_answer(msg: types.Message):
    ...
    if users[uid]["finished"]:
        await msg.answer("Тест уже завершён. Нажми «🔁 Пройти тест заново», чтобы пройти снова.", reply_markup=kb_post_result())
        return
    ...
    if step < len(QUESTIONS):
        ...
        second = None
        if len(scores) > 1:
            second = sorted(scores.items(), key=lambda x: x[1], reverse=True)[1][0]
        ...
        text = (
            f"✨ Твой психотип: *{primary}* ({p['alias']})\n\n"
            f"_О тебе_: {p['desc']}\n\n"
        ...
        await msg.answer(text, parse_mode="Markdown", reply_markup=kb_post_result())

@dp.message_handler(lambda m: m.text in ["✨ Узнать больше", "💞 Партнёрский архетип", "🌿 Психогигиена"])
async def post_menu(msg: types.Message):
    ...
    if msg.text == "✨ Узнать больше":
        ...
        await msg.answer(f"💞 Тебя уравновешивает: {p['partner']}", reply_markup=kb_post_result())
    elif msg.text == "🌿 Психогигиена":
        await msg.answer(f"🌿 Психогигиена для *{arch}*:\n{p['hygiene']}", parse_mode="Markdown", reply_markup=kb_post_result())
...
async def fallback(msg: types.Message):
    ...
    if not users[uid]["finished"]:
        step = users[uid]["step"]
        if step < len(QUESTIONS):
            await msg.answer("Пожалуйста, отвечай буквами A–F 😊\n\n" + QUESTIONS[step], reply_markup=kb_answers())
        else:
            await msg.answer("Нажми /start чтобы начать заново.")
    else:
        await msg.answer("Выбери пункт в меню ниже 👇", reply_markup=kb_post_result())

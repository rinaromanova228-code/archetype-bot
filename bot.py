...
# Вопросы с вариантами ответов (A–H)
questions = [
...
# Описание архетипов
archetypes = {
    "Мудрец": "Ты стремишься к знаниям и пониманию мира. Сила – в разуме.",
    "Воин": "Ты борец, который защищает свои ценности и идёт вперёд.",
    "Творец": "Ты вдохновляешь мир через творчество и идеи.",
    "Заботливый": "Ты даришь тепло и заботу, твоя сила – в поддержке.",
    "Лидер": "Ты умеешь вести за собой и вдохновлять людей.",
    "Искатель": "Ты вечно ищешь новые пути, свобода – твоя ценность.",
    "Маг": "Ты преобразуешь мир, соединяя невидимое и видимое.",
    "Гедонист": "Ты наслаждаешься жизнью и учишь этому других."
}

# Данные пользователей
user_data = defaultdict(lambda: {"step": 0, "counts": defaultdict(int)})

@dp.message_handler(commands=["start"])
async def start(message: types.Message):
    user_id = message.from_user.id
    user_data[user_id]["step"] = 0
    user_data[user_id]["counts"].clear()
    await message.answer("Привет 🌸\nДавай узнаем твой архетип!\nОтвечай на вопросы, выбирая A–H.")
...
async def send_question(user_id, message):
    ...
    if step < len(questions):
        ...
        keyboard.add("A", "B", "C", "D", "E", "F", "G", "H")
        await bot.send_message(user_id, questions[step], reply_markup=keyboard)
    else:
        await show_result(user_id, message)

@dp.message_handler(lambda message: message.text.upper() in answers.keys())
async def handle_answer(message: types.Message):
    user_id = message.from_user.id
    step = user_data[user_id]["step"]
    if step < len(questions):
        choice = message.text.upper()
        archetype = answers[choice]
        user_data[user_id]["counts"][archetype] += 1
        user_data[user_id]["step"] += 1
        await send_question(user_id, message)
    else:
        await show_result(user_id, message)

async def show_result(user_id, message):
    counts = user_data[user_id]["counts"]
    if not counts:
        await message.answer("Ты не ответил ни на один вопрос 😅")
        return
    result = max(counts, key=counts.get)
    description = archetypes[result]
    await message.answer(f"✨ Твой архетип: *{result}*\n\n{description}", parse_mode="Markdown")
    user_data[user_id]["step"] = 0
    user_data[user_id]["counts"].clear()

if __name__ == "__main__":
    print("✅ Новый код успешно запущен")
